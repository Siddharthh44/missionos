...
_The north star. Everything else flows from this._
...
_The blueprint your AI agent needs to make technical decisions without guessing._
...
_Every page, every click, every path — mapped before a single screen is built._
...
_So the AI doesn't just build something functional — it builds something you'd actually use._
...
_How your data is stored, structured, and secured — defined before the AI writes a single migration._
...
| --- | --- |
| **Table: users** | id (uuid), email (text), name (text), created_at (timestamp), role (text) |
| --- | --- |
| **Table: \[your table\]** | id, \[columns\], user_id (FK → users.id), created_at |
| --- | --- |
...
| --- | --- |
| **File Storage** | e.g. Supabase Storage — /avatars/{user_id}, /uploads/{user_id} |
| --- | --- |
...
_The exact order to build so the AI never skips a foundation layer._
...
_The 10 minutes you spend on these documents saves hours of fixing broken code, re-explaining context, and undoing bad architectural decisions._